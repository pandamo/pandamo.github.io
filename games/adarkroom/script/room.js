	...
	updateStoresView: function () {
		...
		for (var k in $SM.get('stores')) {
			...
			if (row.length === 0) {
				...
				$('<span>').addClass('row_num').text(Math.floor(num)).appendTo(val);
				// cheat button: +1000 of this resource on click
				$('<span>')
					.addClass('row_add')
					.text('+')
					.data('resource', k)
					.on('click', function () {
						$SM.add('stores["' + $(this).data('resource') + '"]', 1000);
					})
					.appendTo(val);
				...
				$('div#' + row.attr('id') + ' > div.row_val > span.row_num', location).text(Math.floor(num));
			}
		}
