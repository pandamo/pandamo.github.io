(function () {
  var Engine = window.Engine = {
    ...
    exportImport: function () {
      var CloudSave = Engine.CloudSave;
      ...
      Events.startEvent({
        ...
        scenes: {
          start: {
            text: [
              _('save your game to the cloud, or restore it from the cloud.'),
              _('you will get a 12-letter save key. it is the only way to restore, so keep it safe.')
            ],
            buttons: {
              'save': {
              ...
              'load': {
                text: _('restore from cloud'),
                nextScene: { 1: 'inputKey' }
              },
              'export': {
                text: _('export / import code'),
                nextScene: { 1: 'codeMenu' }
              },
              'cancel': {
                text: _('cancel'),
                nextScene: 'end'
          ...
          'saved': {
            text: [
              _('your save key (12 lowercase letters):'),
              _('keep it safe. anyone with this key can restore your save.')
            ],
            textarea: CloudSave.lastKey,
            readonly: true,
            onLoad: function () { Engine.event('progress', 'cloud save'); },
            buttons: {
              'copy': {
                text: _('copy'),
                nextScene: 'end',
                onChoose: function () {
                  CloudSave.copyText(CloudSave.lastKey);
                  Engine.disableSelection();
                }
              },
              'done': {
          ...
          'inputKey': {
            text: [_('enter your 12-letter save key.')],
            textarea: '',
            onLoad: Engine.enableSelection,
            buttons: {
              'restore': {
                text: _('restore'),
                onChoose: function (key) {
                  key = key ? key.trim() : '';
                  if (!CloudSave.KEY_PATTERN.test(key)) {
                    Events.loadScene('invalidKey');
                    return;
                  }
                  CloudSave.loadGame(key, function (err) {
                    if (err) {
                      Events.loadScene('loadFail');
                    }
                  });
                }
              },
          ...
          'invalidKey': {
            text: [_('the save key must be exactly 12 lowercase letters.')],
            buttons: {
              'ok': {
                text: _('ok'),
                nextScene: { 1: 'inputKey' }
          ...
          'saveFail': {
            text: [_('error while saving to the cloud. please check your connection and try again.')],
            buttons: {
              'retry': {
                text: _('retry'),
                nextScene: { 1: 'start' }
          ...
          'loadFail': {
            text: [_('error while restoring. please check the key and your connection.')],
            buttons: {
              'retry': {
                text: _('retry'),
                nextScene: { 1: 'inputKey' }
