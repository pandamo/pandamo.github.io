...
var Events = {
	...
	recallDelay: function(stateName, target){
		...
		for(var i in state){
			// skip/clean cleared values (null / sparse array slots) so stale
			// saves don't crash or wipe the whole branch
			if(state[i] == null){
				$SM.remove(stateName +'["'+ i +'"]');
				continue;
			}
			if(typeof(state[i]) == 'object'){
				// only descend when the matching event object still exists;
				// otherwise drop the stale branch (no handler left to resume)
				if(target != null && typeof target[i] != 'undefined'){
					Events.recallDelay(stateName +'["'+ i +'"]', target[i]);
				} else {
					$SM.remove(stateName +'["'+ i +'"]');
				}
			} else {
				if(target != null && typeof target[i] == 'function'){
					target[i]();
				} else {
					$SM.remove(stateName);
				}
